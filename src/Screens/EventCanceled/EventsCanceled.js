...
const EventsCanceled = () => {
    ...
    const getRequestedEvent = async () => {
        ...
        axios.get(`https://plansaround-backend.vercel.app/api/mobile/my-event/event-requested?status=CANCELED`, { headers }, {
            "status": "ALL"
        })
            .then((res) => {
                console.log(res, 'eventDetail on event CANCELED screen')
    ...
    return (
        <WrapperContainer>
            <HeaderBack mainText='Canceled Event' style={{ backgroundColor: '#fff', paddingHorizontal: moderateScale(10) }} />
                ...
                <View style={{ flex: 1 }}>
                    {
                        LoadEvents ? <Loader /> :
                            <FlatList
                                ListEmptyComponent={<View style={{ flex: 1, height: height, width: width, justifyContent: 'center', alignItems: 'center', }}><Text style={{ fontSize: scale(15), color: '#4F4F4F', fontWeight: '700' }}>There is no canceled events.</Text></View>}
                                data={Event}
