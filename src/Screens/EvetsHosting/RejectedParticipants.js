...
// create a component
const RejectedParticipants = ({ route, navigation }) => {
    ...
    const getApprovedParticipants = async () => {
        ...
        axios.get(`https://plansaround-backend.vercel.app/api/mobile/my-event/event/${Eid}/booking-request?status=REJECTED`, { headers })
            .then((res) => {
                console.log(res, 'Rejected participants')
    ...
    return (
        <WrapperContainer>
            <HeaderBack mainText='Rejected Participants' style={{ backgroundColor: '#fff', paddingHorizontal: moderateScale(10) }} />
                    ...
                    <View style={{ flex: 1 }}>
                        {
                            LoadEvents ? <Loader /> :
                                <FlatList
                                    ListEmptyComponent={<View style={{ flex: 1, height: height, width: width, justifyContent: 'center', alignItems: 'center', }}><Text style={{ fontSize: scale(15), color: '#4F4F4F', fontWeight: '700' }}>There is no rejected participants.</Text></View>}
                                    data={Event}
...
export default RejectedParticipants;
